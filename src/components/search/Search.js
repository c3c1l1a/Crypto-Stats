...
const Search = ({ setFiltedCoins }) => {
  const coins = useSelector((state) => state.coins);

  const onChange = (e) => {
    const filteredCoins = coins.filter((item) => item.name.toLowerCase()
      .includes(e.target.value.toLowerCase())
      || item.symbol.toLowerCase().includes(e.target.value.toLowerCase()));

    setFiltedCoins(filteredCoins);
  };

  return (
    <div className="search">
      <input className="search-text" type="text" placeholder="Search" onChange={onChange} />
...
Search.propTypes = {
  setFiltedCoins: PropTypes.func.isRequired,
};
